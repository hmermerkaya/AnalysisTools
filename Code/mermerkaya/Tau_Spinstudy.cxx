...
void  Tau_SpinExample::Configure(){
  ...
  mu_Pt=HConfig.GetTH1D(Name+"_mu_Pt","mu_Pt",30,0.,70.,"Pt_Mu_gen","Events");
  pi_Pt=HConfig.GetTH1D(Name+"_pi_Pt","pi_Pt",30,0.,70.,"Pt_Pi_gen","Events");
...
void  Tau_SpinExample::doEvent(){
  ...
  if(status){
	...
	mu_Pt.at(t).Fill(X_LV.Pt(),w); 
	if(verbose)std::cout<<"Muon_gen_Pt=="<< X_LV.Pt() <<std::endl;
	if(verbose){
	  for(int i=0;i<Ntp->NMCTauDecayProducts(tau_idx);i++){
   ...
   if(Ntp->hasSignalTauDecay(PDGInfo::Z0,Boson_idx,TauDecay::JAK_PION,tau_idx)){
	...
	pi_Pt.at(t).Fill(X_LV.Pt(),w);
	if(verbose)std::cout<<"Pion_gen_Pt=="<< X_LV.Pt() <<std::endl;
...
double  Tau_SpinExample::Zstoa(double zs){
  ...
  return a;
    ...
    // if(Ntp->hasSignalTauDecay(PDGInfo::Z0,Boson_idx,tau1_idx,tau2_idx) ) {
    //   if((Ntp->MCTau_JAK(tau1_idx)==TauDecay::JAK_PION && Ntp->MCTau_JAK(tau2_idx)==TauDecay::JAK_MUON) || (Ntp->MCTau_JAK(tau1_idx)==TauDecay::JAK_MUON && Ntp->MCTau_JAK(tau2_idx)==TauDecay::JAK_PION)) {
